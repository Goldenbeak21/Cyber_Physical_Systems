...
def detect_cusum(x,threshold=1, drift=0, ending=False, show=True, ax=None):
    ...
    if tai.size and ending:
        _, tai2, _, _, _= detect_cusum(x[::-1], threshold, drift, show=False)
        taf = x.size - tai2[::-1] - 1
...
req = np.zeros((900,))
for i in range(6):
    req[i*150:(i+1)*150] = pressure[i*300:i*300+150:,0]
...
label = np.zeros((900,))
thr = np.arange(0,3000,2)
fpr = np.zeros((thr.shape[0],))
for i in range(thr.shape[0]):
    ta, tai, taf, amp, gp = detect_cusum(req, 2000999000, 10, True, False)
    t = thr[i]
    pred = np.zeros((gp.shape[0],))
    pred = pred.astype('int')
    pred = (gp > t)
    fpr[i] = np.sum(pred)/900
...
ta, tai, taf, amp, gp = detect_cusum(req, 1600, 1, True, True)
ta, tai, taf, amp = detect_cusum(power, 2, .02, True, True)
ta, tai, taf, amp = detect_cusum(temperature, 2, .02, True, True)
